...
namespace App\Datatables\Vendor\htmLawed;
...
$C['anti_link_spam'] = (isset($C['anti_link_spam']) && is_array($C['anti_link_spam']) && count($C['anti_link_spam']) == 2 && (empty($C['anti_link_spam'][0]) or \App\Datatables\Vendor\htmLawed\htmLawed::hl_regex($C['anti_link_spam'][0])) && (empty($C['anti_link_spam'][1]) or App\Datatables\Vendor\htmLawed\htmLawed::hl_regex($C['anti_link_spam'][1]))) ? $C['anti_link_spam'] : 0;
...
$GLOBALS['C'] = $C;
$S = is_array($S) ? $S : App\Datatables\Vendor\htmLawed\htmLawed::hl_spec($S);
...
if($C['cdata'] or $C['comment']){$t = preg_replace_callback('`<!(?:(?:--.*?--)|(?:\[CDATA\[.*?\]\]))>`sm', 'App\Datatables\Vendor\htmLawed\htmLawed::hl_cmtcd', $t);}
$t = preg_replace_callback('`&amp;([A-Za-z][A-Za-z0-9]{1,30}|#(?:[0-9]{1,8}|[Xx][0-9A-Fa-f]{1,7}));`', 'App\Datatables\Vendor\htmLawed\htmLawed::hl_ent', str_replace('&', '&amp;', $t));
...
// main
$t = preg_replace_callback('`<(?:(?:\s|$)|(?:[^>]*(?:>|$)))|>`m', 'App\Datatables\Vendor\htmLawed\htmLawed::hl_tag', $t);
...
foreach($aA as $k=>$v){
 if(((isset($C['deny_attribute']['*']) ? isset($C['deny_attribute'][$k]) : !isset($C['deny_attribute'][$k])) && (isset($aN[$k][$e]) or (isset($aNU[$k]) && !isset($aNU[$k][$e]))) && !isset($rl['n'][$k]) && !isset($rl['n']['*'])) or isset($rl[$k])){
  ...
  if($k == 'style' && !$C['style_pass']){
   ...
   $v = preg_replace_callback('`(url(?:\()(?: )*(?:\'|"|&(?:quot|apos);)?)(.+?)((?:\'|"|&(?:quot|apos);)?(?: )*(?:\)))`iS', 'App\Datatables\Vendor\htmLawed\htmLawed::hl_prot', $v);
...
$C['hook'] = 'App\Datatables\Vendor\htmLawed\htmLawed::kses_hook';
$C['schemes'] = '*:'. implode(',', $p);
